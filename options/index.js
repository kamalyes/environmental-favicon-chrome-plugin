...
const temp = (el = {}) => `<div class="items-color">
        <div class="color-current" tabindex="-1">
            <div class="color-list">
                ${colors
                  .map(
                    (color) =>
                      '<button class="color-pane" style="--color: ' +
                      color +
                      '"></button>'
                  )
                  .join("")}
    ...
    <div class="items-name">
        <input class="input" required data-type="name" placeholder="name" value="${
          el.name || ""
        }">
    </div>
...
chrome.storage.sync.get("options", ({ options = default_options }) => {
  list.innerHTML = options
    .map(
      (el) => `<div class="items" style="--color:${el.color}">
        ${temp(el)}
    </div>`
    )
    .join("");
...
list.addEventListener("click", function (ev) {
  if (ev.target.className === "color-pane") {
    ...
    item.style.setProperty(
      "--color",
      ev.target.style.getPropertyValue("--color")
    );
  }
...
list.addEventListener("input", function (ev) {
  if (ev.target.dataset.type === "name") {
    ev.target.parentNode.parentNode.querySelector(".env").innerText =
      ev.target.value;
  }
...
add.addEventListener("click", function () {
  ...
  items.style.setProperty(
    "--color",
    colors[list.childElementCount % colors.length]
  );
...
save.addEventListener("click", function () {
  const invalidElAll = [
    ...document.querySelectorAll("input.input:invalid, div.input:empty"),
  ];
